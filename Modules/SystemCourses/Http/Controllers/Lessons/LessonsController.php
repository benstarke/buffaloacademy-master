...
class LessonsController extends Controller
{
    ...
    {
        $lessons = DB::table('par_lessons')->latest()->get();

        // Add duration field
        $lessons = $lessons->map(function ($lesson) {
            $createdDate = Carbon::parse($lesson->created_at);
            $lesson->duration = $createdDate->diffForHumans();
            return $lesson;
        });
    ...
    {
        $lesson = DB::table('par_lessons')
            ->join('par_users', 'par_lessons.created_by', '=', 'par_users.id')
            ->select('par_lessons.*', 'par_users.name_en as author')
            ->where('par_lessons.id', $id)
            ->first();
        ...
        // Calculate the duration since the lesson was created using Carbon
        $createdDate = Carbon::parse($lesson->created_at);
        $lesson->duration = $createdDate->diffForHumans(); // Add duration field
    ...
    public function store(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'title' => 'required|string|max:255|unique:par_lessons',
        ...
        try {
            ...
            $lessonId = DB::table('par_lessons')->insertGetId([
    ...
    {
        $lesson = DB::table('par_lessons')->find($id);
        ...
        $validator = Validator::make($request->all(), [
            'title' => [
                ...
                'string',
                'max:255',
                Rule::unique('par_lessons')->ignore($id),
            ],
            'course_curriculum_id' => 'required|integer',
            'description' => 'nullable|string',
            'notes' => 'nullable|string',
        ...
        try {
            ...
            $previousData = (array)$lesson;

            // Update the lesson
            DB::table('par_lessons')
                ->where('id', $id)
                ->update([
                    'title' => $request->input('title'),
                    'course_curriculum_id' => $request->input('course_curriculum_id'),
            ...
            // Fetch the updated lesson data for audit trail
            $updatedLesson = DB::table('par_lessons')->find($id);
            ...
            $auditTrailData = [
                ...
                'current_tabledata' => json_encode($updatedLesson),
                'user_id' => $userEmail,
            ];
    ...
    public function destroy($id): JsonResponse
    {
        ...
        try {
            ...
            // Delete the lesson
            DB::table('par_lessons')->where('id', $id)->delete();
    ...
    public function destroyMultiple(Request $request): JsonResponse
    {
        ...
        $lessons = DB::table('par_lessons')->whereIn('id', $ids)->get();
        ...
        try {
            ...
            $deletedCount = DB::table('par_lessons')->whereIn('id', $ids)->delete();
