...
export class AuthService {
  ...
  logout(): Promise<any> {
    ...
    return of(null).pipe(
      // delay(1000),
      switchMap(() =>
        this.http.post('http://127.0.0.1:8000/api/authentication-sub-system/instructor/logout', {}, {
          headers: {
  ...
  login(email: string, password: string, rememberMe: boolean): Observable<any> {
    return this.http.post('http://127.0.0.1:8000/api/authentication-sub-system/instructor/login', {
      email,
      password,
      remember: rememberMe
    }).pipe(
      tap((response: any) => {
        ...
        localStorage.setItem('instructor_id', response.authorization.instructor_id);
