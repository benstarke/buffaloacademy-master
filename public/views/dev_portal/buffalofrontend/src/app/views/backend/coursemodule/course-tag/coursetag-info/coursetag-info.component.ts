...
@Component({
  ...
  templateUrl: './coursetag-info.component.html',
  styleUrl: './coursetag-info.component.css'
})
export class CoursetagInfoComponent implements OnInit {
    ...
    this.tagService.getTags().subscribe(
      (response: any) => {
        ...
        if (response.success) {
          ...
          this.showToast('Tags loaded successfully!', 'info');
        } else {
          this.tags = [];
  ...
  exportToExcel(e: DxDataGridTypes.ExportingEvent, selectedData: any[]) {
    ...
    const worksheet = workbook.addWorksheet('CourseTags');
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'CourseTags.xlsx');
      });
    });
  }  
