...
export class SigninComponent implements OnInit, OnDestroy  {
  ...
  login(event: Event) {
    ...
    if (!this.emailExists) {
      this.toastr.error('Email does not exist. Redirecting to register page...', '', { timeOut: 2000 }).onHidden.subscribe(() => {
        this.router.navigate(['/admin-buffalo/aunthentication-sign-up']);
      });
      return;
    }
    ...
    this.authService.login(this.email, this.password, this.rememberMe).subscribe(
      (response: any) => {
        ...
        // Extract instructor_id and token from the response
        const instructorId = response.authorization.instructor_id;
        const email = response.authorization.email;
        ...
        this.cd.detectChanges(); // Manually trigger change detection
        this.router.navigate(['/admin-buffalo/instructor-dashboard']);
      },
      (error: any) => {
  ...
  checkEmailExists(email: string) {
    this.http.post('http://127.0.0.1:8000/api/authentication-sub-system/student/check-email', { email }).subscribe(
      (response: any) => {
        if (response.exists) {
          // Email exists in the students table, redirect to student login page
          const toast = this.toastr.info('Please use the student login page to access your portal.', '', { timeOut: 2000 });
          ...
          this.http.post('http://127.0.0.1:8000/api/authentication-sub-system/instructor/check-email', { email }).subscribe(
            (instructorResponse: any) => {
              if (instructorResponse.exists) {
                // Email exists in the instructors table, do nothing
              } else {
                ...
                this.http.post('http://127.0.0.1:8000/api/authentication-sub-system/user/check-email', { email }).subscribe(
                  (userResponse: any) => {
                    if (userResponse.exists) {
                      // Email exists in the users table, do nothing
                    } else {
                      // Email does not exist in any table, redirect to home page
  ...
  onFuncRecoverPasswordRequest() {
    ...
    this.http.post('http://127.0.0.1:8000/api/authentication-sub-system/instructor-password-reset-via-email/password/email', { email })
      .pipe(
