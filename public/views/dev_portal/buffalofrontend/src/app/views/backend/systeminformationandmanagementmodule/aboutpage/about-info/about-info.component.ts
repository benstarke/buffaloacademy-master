...
export class AboutInfoComponent implements OnInit {
  ...
  @ViewChild(DxDataGridComponent, { static: false }) AboutGrid!: DxDataGridComponent;
    ...
    this.AboutService.getAbouts().subscribe(
      (response: any) => {
        ...
        if (response.success) {
          ...
          this.showToast('About loaded successfully!', 'info');
        } else {
          this.About = [];
      ...
      (error: any) => {
        ...
        this.showToast('Error fetching About', 'error');
      }
    );
  }
  ...
  exportToExcel(e: DxDataGridTypes.ExportingEvent, selectedData: any[]) {
    ...
    const worksheet = workbook.addWorksheet('About');
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'About.xlsx');
      });
    });
  }  

  exportToCSV(selectedData: any[]) {
    const dataToExport = selectedData.length > 0 ? selectedData : this.About;
    const csvData = unparse(dataToExport.map(cat => ({ name: cat.name, description: cat.description })));
    const csvBlob = new Blob([csvData], { type: 'text/csv;charset=utf-8;' });
    saveAs(csvBlob, 'About.csv');
  }

  selectionChanged(e: any): void {
    this.selectedAbout = e.selectedRowsData; // Capture selected rows
  }
