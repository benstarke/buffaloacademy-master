...
@Component({  
  ...
  templateUrl: './coursecategory-mgmt.component.html',
  styleUrl: './coursecategory-mgmt.component.css'
})

export class CoursecategoryMgmtComponent implements OnInit {
...
onMenuItemClick(event: any): void {
  ...
  onAddCategorySubmit(addAnother: boolean): void {
    if (this.addCategoryForm.valid) {
      const newCategoryName = this.addCategoryForm.get('category_name')?.value.trim();
  
      // Frontend validation for unique category name
      const categoryExists = this.categories.some(category => 
        category.name && category.name.toLowerCase() === newCategoryName.toLowerCase()
      );
      if (categoryExists) {
