...
export class SignupComponent implements OnInit, OnDestroy  {
  ...
  instructorId: number | null = null; // Add instructor ID property
  ...
  register(event: Event) {
    ...
    of(null).pipe(
      ...
      switchMap(() => 
        this.http.post('http://127.0.0.1:8000/api/authentication-sub-system/instructor/register', {
          name: this.name,
          email: this.email,
      ...
      (response: any) => {
        ...
        this.instructorId = response.instructorId; // Store the instructor ID
      }
    ).add(() => {
...
resendVerificationEmail() {
  ...
  this.http.get(`http://127.0.0.1:8000/api/authentication-sub-system/instructor/email/resend/${this.instructorId}`)
    .pipe(
      catchError(error => {
        ...
        if (response.exists) {
          ...
          toast.onHidden.subscribe(() => {
            ...
            redirectToast.onHidden.subscribe(() => {
              this.router.navigate(['/app-instructor-auth/authentication-sub-system/instructorLogin']);
            });
          });
        } else {
          this.emailExists = false;
