...
export class LayoutComponent implements OnInit {
  ...
  navigate(path: string | undefined) {
    if (path) {
      this.router.navigate([`/admin-buffalo/content-management/dashboard-blog-tags/${path}`]);
    } else {
      this.router.navigate(['/admin-buffalo/content-management/dashboard-blog-tags']);
    }
  }
