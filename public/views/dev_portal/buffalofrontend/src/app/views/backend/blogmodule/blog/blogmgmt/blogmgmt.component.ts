...
export class BlogmgmtComponent implements OnInit {
  ...
  step2Visible = false;
  step3Visible = false;
  isPopupOpen = false; // New flag to manage popup visibility
  ...
  // Optional: Functions to get selected values for preview
  getSelectedTags() {
    const tagsId = this.addBlogForm.get('tags_id');
    return tagsId ? this.tagsDataSource.find(tag => tag.id === tagsId.value)?.name : null;
  }

  getSelectedCategory() {
      const categoryId = this.addBlogForm.get('blog_categories_id');
      return categoryId ? this.categoriesDataSource.find(cat => cat.id === categoryId.value)?.name : null;
  }

  getSelectedStatus() {
      const statusId = this.addBlogForm.get('blog_status_id');
      return statusId ? this.statusDataSource.find(status => status.id === statusId.value)?.name : null;
  }
  ...
  openStep1() {
      ...
      this.isPopupOpen = true; // Ensure popup is open
  }
  ...
  onAddBlogClick(): void {
      ...
      // this.openStep1(); // Directly call openStep1 to initialize Step 1
  }

  onPopupHidden(): void {
      this.fetchBlogs(); // Refresh the blog list after popup hidden
      this.isPopupOpen = false; // Reset popup visibility flag
  }
